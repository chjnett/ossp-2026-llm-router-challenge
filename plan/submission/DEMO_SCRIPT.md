...
| 5 | 1:30-2:00 | 터미널: `self-check` → 등급별 점수·예산 사용률 표 | "공식 채점기로 검증합니다. fast 0.656, balanced 0.685, premium 0.721, 가중 최종 0.684. 세 등급 모두 예산을 통과했습니다." |
...
for t in fast balanced premium; do
  docker run --rm --network none --cpus 2 --memory 2g \
    -v $PWD/input:/challenge/input:ro -v $PWD/out:/challenge/output \
    ghcr.io/chjnett/ossp-router:submission --tier $t
done
...
- 가중 점수 0.6839 (fast 0.6557 · balanced 0.6847 · premium 0.7207)
...
- 파레토: 보수 q0.8(0.681) → 채택 q0.68(0.684) → 과공격 q0.65(0.685) 곡선
